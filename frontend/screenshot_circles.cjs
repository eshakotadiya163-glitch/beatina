...
(async () => {
  ...
  await local.evaluate(() => window.scrollTo(0, 600));
  await new Promise(r => setTimeout(r, 1000));
  await local.screenshot({ path: 'circles_local.png' });
  ...
  await ref.evaluate(() => window.scrollTo(0, 600));
  await new Promise(r => setTimeout(r, 1000));
  await ref.screenshot({ path: 'circles_ref.png' });
